(() => {
  ...
  function nocScore() {
    ...
    $('calcNocHint').textContent =
      `TEER ${teer}: ${teerPts} pts · Occupational category ${cat}: ${catPts} pts`;
  ...
  function educationScore() {
    ...
    document
      .querySelectorAll('#calcEducationChoices input:checked')
      .forEach((box) => {
    ...
    $('calcHighestEdu').innerHTML =
      `Highest education used: <b>${name} — ${high} pts</b>`;
  ...
  function update() {
    ...
    $('calcProgress').style.width =
      `${Math.min(100, total / 130 * 100)}%`;
  ...
  document
    .querySelectorAll('#calculatorPanel input, #calculatorPanel select')
    .forEach((el) => {
  ...
  $('calcReset').addEventListener('click', () => {
    ...
    // This prevents Language from automatically receiving 5 points
    document
  ...
  $('calcSubmitCommunity').addEventListener('click', () => {
    ...
    const regionOption =
      $('calcRegion').selectedOptions[0];

    if (regionOption?.dataset.community) {
      $('shareRegion').value =
        regionOption.dataset.community;
    }

    showTab('community');

    const shareCard =
      $('shareForm').closest('.card');
